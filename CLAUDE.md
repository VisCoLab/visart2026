...
### Generated dataset
The full synthetic dataset — **24,760 RGB images** (4,952 MET paintings × 5 cameras, 512²) — lives in backed-up `project_data`, **not** in this repo: `/mnt/storage_6/project_data/pl0896-03/visart-dataset/` (flat layout, contiguous folders `0..4951`; folders `0..4897` ↔ `data/paintings_train_images_unique.json`, `4898..4951` ↔ `data/synth_gen/new_to_render.json`; per-folder `metadata.json` has the definitive source path — see that directory's `README.md`). Regenerate the base with `sbatch slurm/render_array.sbatch` — a 5-shard SLURM array over the unique manifest `data/paintings_train_images_unique.json` (4,898 images, the one-per-object filter of `paintings_train_images.json`) — merging shards by global index (`shard*1000 + datapoint`). Extra sources (e.g. the `data/synth_gen/` strict/broad manifests) are deduped against what's already rendered and appended at the next free index.
